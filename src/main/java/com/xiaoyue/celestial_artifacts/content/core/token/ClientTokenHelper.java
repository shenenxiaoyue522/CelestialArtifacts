...
public class ClientTokenHelper {
	...
	@Nullable
	public static <T extends BaseTickingToken> T get(TokenKey<T> facet, @Nullable Level level) {
		if (level == null) return null;
		var player = Minecraft.getInstance().player;
		if (player == null) return null;
		return Wrappers.get(() -> L2LibReg.CONDITIONAL.type().getOrCreate(player).getData(facet));
	}
	...
	public static boolean flag(@Nullable Level level, String flag) {
		...
		var player = Minecraft.getInstance().player;
		if (player == null) return false;
		return PlayerFlagData.hasFlag(player, flag);
	}
