...
public record HurtTargetEffectFacet(@Nullable DoubleSupplier chance, Supplier<Holder<MobEffect>> eff, IntSupplier duration, IntSupplier amplifier)
		implements SingleLineText, CAAttackToken {

	public static HurtTargetEffectFacet of(DoubleSupplier chance, Supplier<Holder<MobEffect>> eff, IntSupplier duration, IntSupplier amplifier) {
		return new HurtTargetEffectFacet(chance, eff, duration, amplifier);
	}

	public static HurtTargetEffectFacet of(Supplier<Holder<MobEffect>> eff, IntSupplier duration, IntSupplier amplifier) {
	...
	public void onPlayerHurtTarget(Player player, DamageData.Offence cache) {
		if (chance == null || CAAttackToken.chance(player, chance.getAsDouble())) {
			cache.getTarget().addEffect(new MobEffectInstance(eff.get(), duration.getAsInt() * 20, amplifier.getAsInt()));
		}
	}
