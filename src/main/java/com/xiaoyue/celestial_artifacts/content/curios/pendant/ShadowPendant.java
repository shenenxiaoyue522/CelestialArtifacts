...
public class ShadowPendant implements MultiLineText, CAAttackToken {
	...
	private static double damageHealFactor() {
		return CAModConfig.SERVER.pendant.shadowPendantDamageHeal.get();
	}

	private static double damageBonusFactor() {
		return CAModConfig.SERVER.pendant.shadowPendantDamageBonus.get();
	}

	private static double damageReductionFactor() {
		return CAModConfig.SERVER.pendant.shadowPendantDamageReduction.get();
	}

	private static int light() {
		return CAModConfig.SERVER.pendant.shadowPendantLightLevel.get();
	}
	...
	public void onPlayerHurtTarget(Player player, DamageData.Offence cache) {
		...
		if (brightness < light()) {
			...
			cache.addHurtModifier(DamageModifier.multTotal((float) (1 + (add * damageBonusFactor())),
					CAItems.SHADOW_PENDANT.getId().withSuffix("_attack_bonus")));
		}
	}

	@Override
	public void onPlayerDamageTargetFinal(Player player, DamageData.DefenceMax cache) {
		player.heal((float) (cache.getDamageFinal() * damageHealFactor()));
	}
		...
		if (player.level() instanceof ServerLevel sl) {
			...
			if (brightness < light()) {
				...
				cache.addDealtModifier(DamageModifier.multTotal((float) (1 - (add * damageReductionFactor())),
						CAItems.SHADOW_PENDANT.getId().withSuffix("_damage_reduce")));
			}
		}
