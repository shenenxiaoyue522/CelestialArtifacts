...
public class LeechScabbard implements CAAttackToken, SingleLineText {
	...
	private static Holder<MobEffect> getEff() {
		return CCEffects.BLADE_MODIFIER.holder();
	}

	private static double getHeal() {
		return CAModConfig.SERVER.back.leechScabbardHealFactor.get();
	}
	...
	public void onPlayerDamageTargetFinal(Player player, DamageData.DefenceMax cache) {
		...
		if (player.hasEffect(getEff())) {
			player.heal(cache.getDamageFinal() * (float) getHeal());
		}
	}
