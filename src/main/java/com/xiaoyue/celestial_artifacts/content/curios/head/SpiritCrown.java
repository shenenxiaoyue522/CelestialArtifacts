...
public class SpiritCrown implements MultiLineText, CAAttackToken {
	...
	private static double proj() {
		return CAModConfig.SERVER.head.spiritCrownArrowDamageMultiplier.get();
	}

	private static double dist() {
		return CAModConfig.SERVER.head.spiritCrownDistanceDamage.get();
	}

	private static int count() {
		return CAModConfig.SERVER.head.spiritCrownMaxEntityCount.get();
	}
	...
	public void onPlayerHurtTarget(Player player, DamageData.Offence cache) {
		...
		if (entities.size() <= count()) {
			cache.addHurtModifier(DamageModifier.multTotal((float) (1 + proj()), CAItems.SPIRIT_CROWN.getId().withSuffix("attack_with_entities")));
		}
		float distance = player.distanceTo(cache.getTarget());
		cache.addHurtModifier(DamageModifier.multTotal((float) (1 + distance * dist()), CAItems.SPIRIT_CROWN.getId().withSuffix("attack_with_dis")));
	}
