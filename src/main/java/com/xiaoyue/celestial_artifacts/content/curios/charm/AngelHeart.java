...
public class AngelHeart implements MultiLineText, TickFacet, CAAttackToken {
	...
	private static int removeInterval() {
		return CAModConfig.SERVER.charm.angelHeartRemoveInterval.get();
	}

	private static int bloodInterval() {
		return CAModConfig.SERVER.charm.angelHeartBloodInterval.get();
	}

	private static int healAmount() {
		return CAModConfig.SERVER.charm.angelHeartHealAmount.get();
	}
	...
	public void tick(LivingEntity entity, ItemStack stack) {
		...
		if (entity.tickCount % (removeInterval() * 20) == 0) {
			List<Holder<MobEffect>> list = entity.getActiveEffects().stream().map(MobEffectInstance::getEffect)
					.filter(effect -> effect.value().getCategory().equals(MobEffectCategory.HARMFUL)).toList();
