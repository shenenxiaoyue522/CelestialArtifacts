...
public class PrayerCrown implements MultiLineText, CAAttackToken {
	...
	private static double healAmountFactor() {
		return CAModConfig.SERVER.head.prayerCrownHealAmount.get();
	}

	private static double healChance() {
		return CAModConfig.SERVER.head.prayerCrownHealChance.get();
	}
	...
	public void onPlayerDamagedFinal(Player player, DamageData.DefenceMax cache) {
		if (player.isCrouching()) {
			if (CAAttackToken.chance(player, healChance())) {
				SchedulerHandler.schedule(() -> player.heal((float) (cache.getDamageFinal() * healAmountFactor())));
			}
		}
	}
}
