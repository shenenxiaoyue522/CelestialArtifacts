...
public class GluttonyBadge implements MultiLineText, CAAttackToken {
	...
	private static double protection() {
		return CAModConfig.SERVER.charm.gluttonyBadgeProtection.get();
	}

	private static int duration() {
		return CAModConfig.SERVER.charm.gluttonyBadgeEffectDuration.get();
	}
	...
	public void onPlayerDamaged(Player player, DamageData.Defence cache) {
		...
		cache.addDealtModifier(DamageModifier.multTotal(val, CAItems.GLUTTONY_BADGE.getId()));
	}
