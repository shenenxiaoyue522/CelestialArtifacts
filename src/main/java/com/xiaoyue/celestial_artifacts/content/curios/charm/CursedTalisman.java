...
public class CursedTalisman extends BaseTickingToken {
	...
	private static double crit() {
		return CAModConfig.SERVER.charm.cursedTalismanCritRateAdd.get();
	}

	private static double critDamage() {
		return CAModConfig.SERVER.charm.cursedTalismanCritDamageAdd.get();
	}
	...
	private AttrAdder dmg() {
		...
		return AttrAdder.of("cursed_talisman", L2DamageTracker.CRIT_DMG::holder,
				AttributeModifier.Operation.ADD_VALUE, cta);
	}

	private AttrAdder ctr() {
		double ctr = cursed_talisman_add * crit();
		return AttrAdder.of("cursed_talisman", L2DamageTracker.CRIT_RATE::holder,
				AttributeModifier.Operation.ADD_VALUE, ctr);
	}
	...
	protected void tickImpl(Player player) {
		cursed_talisman_add = ItemUtils.getAllSlotCurseEnchSize(player);
		dmg().tickImpl(player);
		ctr().tickImpl(player);
	}
