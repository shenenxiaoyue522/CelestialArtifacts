...
public class AbyssCore implements SingleLineText, CAAttackToken {
	...
	private static int cooldownFactor() {
		return CAModConfig.SERVER.head.abyssCoreCooldown.get();
	}

	private static int damageReduceFactor() {
		return CAModConfig.SERVER.head.abyssCoreReduce.get();
	}

	private static int damageJudgementFactor() {
		return CAModConfig.SERVER.head.abyssCoreDamageJudgement.get();
	}
	...
	public void onPlayerDamaged(Player player, DamageData.Defence cache) {
		if (!player.getCooldowns().isOnCooldown(CAItems.ABYSS_CORE.get())) {
			cache.addDealtModifier(DamageModifier.nonlinearPre(245, d -> parse(d, player), CAItems.ABYSS_CORE.getId()));
