...
public class KnightShelter implements MultiLineText, TickFacet, ShieldingFeature {
	...
	private static int healTime() {
		return CAModConfig.SERVER.charm.knightShelterHealInterval.get();
	}

	private static double damage() {
		return CAModConfig.SERVER.charm.knightShelterReflection.get();
	}

	@Override
	public void onPlayerBlocked(Player player, LivingShieldBlockEvent event) {
		Entity attacker = event.getDamageSource().getEntity();
		if (attacker != null) {
			SchedulerHandler.schedule(() -> attacker.hurt(player.damageSources().playerAttack(player),
