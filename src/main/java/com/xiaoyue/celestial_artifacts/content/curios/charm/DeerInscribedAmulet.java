...
public class DeerInscribedAmulet extends BaseTickingToken implements CAAttackToken {
    ...
    private static float bonus() {
        return CAModConfig.SERVER.charm.deerInscribedAmuletBlessBonus.get().floatValue();
    }

    private static int time() {
        return CAModConfig.SERVER.charm.deerInscribedAmuletStrengthInterval.get();
    }
    ...
    public void addText(@Nullable Level level, List<Component> list) {
        list.add(TextFacet.wrap(CALang.Charm.DEER_INSCRIBED_AMULET_1.get(TextFacet.num(time()), TextFacet.eff(CAEffects.ENFEEBLED_LACERATION.holder()))));
    ...
    public void onPlayerHurtTarget(Player player, DamageData.Offence cache) {
        ...
        if (chargingTime == time() * 20) {
            EntityUtils.addEct(target, CAEffects.ENFEEBLED_LACERATION.holder(), 100);
            chargingTime = 0;
        }
        if (CurioUtils.isCsOn(player)) {
            int mulBonus = CurioUtils.getCurseAmount(player) == 0 ? 8 : 8 - CurioUtils.getCurseAmount(player);
            cache.addHurtModifier(DamageModifier.multBase(mulBonus * bonus(), CAItems.DEER_INSCRIBED_AMULET.getId()));
        }
    }
}
