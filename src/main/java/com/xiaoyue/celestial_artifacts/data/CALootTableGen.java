...
public enum CALootTableGen implements CALang.Info {
	...
	@SafeVarargs
	CALootTableGen(String def, ResourceKey<LootTable> target, int odds, Supplier<ModularCurio>... item) {
	...
	public static void onLootGen(RegistrateLootTableProvider pvd) {
		pvd.addLootAction(LootContextParamSets.EMPTY, cons -> {
			for (var e : values()) {
				...
				for (var item : e.item) {
					var entry = LootItem.lootTableItem(item.get()).apply(SetItemCountFunction.setCount(ConstantValue.exactly(1))).setWeight(1);
				...
				pool.add(LootItem.lootTableItem(Items.AIR).apply(SetItemCountFunction.setCount(ConstantValue.exactly(0))).setWeight(e.odds));
	...
	public ResourceKey<LootTable> id() {
		return ResourceKey.create(Registries.LOOT_TABLE, CelestialArtifacts.loc("chests/" + name().toLowerCase(Locale.ROOT)));
	}
}
