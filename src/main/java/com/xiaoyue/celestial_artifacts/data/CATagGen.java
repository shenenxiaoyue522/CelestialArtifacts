...
public class CATagGen {
	...
	public static final TagKey<Item> ETCHINGS = ItemTags.create(ResourceLocation.fromNamespaceAndPath(CuriosConstants.MOD_ID, "etching"));

	public static void onItemTagGen(RegistrateItemTagsProvider pvd) {
		var curse = pvd.addTag(REQUIRE_CURSE);
		for (var e : CAItems.ALL_CURIOS) {
			var item = BuiltInRegistries.ITEM.get(CelestialArtifacts.loc(e));
			if (!(item instanceof ModularCurio curio)) continue;
			if (curio.prop.requireCS()) {
				curse.add(curio);
			}
		}
		pvd.addTag(ItemTags.create(ResourceLocation.fromNamespaceAndPath("l2hostility", "no_seal")))
				.add(CAItems.CATASTROPHE_SCROLL.get())
				.addTag(ItemTags.create(ResourceLocation.fromNamespaceAndPath(CuriosConstants.MOD_ID, "etching")));
	}
